...
async function createGraph(
  ...
  const { data: marketRun, error: marketError } = await supabase
    ...
    .insert({
      ...
      output: {
        isMock: true,
        countryCode: "US",
        executiveSummary: "The United States is a viable test market for this workflow product.",
        productCountryFit: "Strong fit with cloud-first B2B SaaS teams.",
        strongestFitSignals: ["High SaaS adoption", "Large technical buyer base"],
        weakestFitSignals: ["Competitive category"],
        relevantCustomerSegments: ["B2B SaaS", "Cloud infrastructure"],
        likelyBuyerRoles: ["Chief Technology Officer", "VP Engineering"],
        localizationRequirements: "English-first positioning with US terminology.",
        languageConsiderations: "Use concise US English.",
        pricingConsiderations: "Lead with operational ROI.",
        paymentProcurementConsiderations: "Support card and invoiced procurement.",
        salesCycleExpectations: "Expect a multi-stakeholder evaluation.",
        preferredEntryMotions: ["Founder-led sales", "Product-led trial"],
        likelyAcquisitionChannels: ["Technical communities", "Targeted outbound"],
        adoptionBarriers: ["Switching cost", "Security review"],
        regulatoryConsiderations: "Validate state-specific obligations.",
        dataProtectionConsiderations: "Document data handling and retention.",
        operationalChallenges: ["Crowded vendor landscape"],
        assumptionsUsed: ["Cloud delivery is supported"],
        unresolvedQuestions: ["Which segment converts fastest?"],
        validationExperiments: ["Run ten buyer interviews"],
        entryRecommendation: "pursue",
        confidence: "high",
        evidenceLimitations: ["Fixture data is deterministic"],
      },
      completed_at: new Date().toISOString(),
    })
    .select()
  ...
  const { data: icp, error: icpError } = await supabase
    ...
    .insert({
      ...
      country_code: "US",
      industry_segments: {
        primary: [{ name: "B2B SaaS", fit: "primary", reasoning: "Cloud-first operations" }],
        secondary: [],
        excluded: [],
        niche: [],
      },
      company_attributes: {
        ...
        maturity: "Growth",
        operatingModel: "Cloud-first",
        geographicPresence: "United States",
        digitalMaturity: "High",
        technologyMaturity: "Modern stack",
        buyingReadiness: "Active evaluation",
      },
      buyer_roles: [
        { title: "Chief Technology Officer", department: "Technology", seniority: "Executive" },
      ],
      pains: ["Manual workflow coordination"],
      desired_outcomes: ["Faster execution"],
      purchase_triggers: ["Engineering team growth"],
      qualification_signals: ["Cloud infrastructure"],
      disqualification_signals: ["No technical operations team"],
      confidence: "high",
      confidence_reason: "Stored project, market and company signals align.",
      created_by: userId,
    })
    .select()
    .single();
